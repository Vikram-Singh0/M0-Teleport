...
abstract contract MYieldToOneStorageLayout {
    ...
    function _getMYieldToOneStorageLocation()
        internal
        pure
        returns (MYieldToOneStorageStruct storage $)
    {
        assembly {
    ...
    Freezable
{
    ...
    ) MExtension(mToken, swapFacility) {}
    ...
    function __MYieldToOne_init(
        ...
        if (yieldRecipientManager == address(0))
            revert ZeroYieldRecipientManager();
    ...
    function setYieldRecipient(
        address account
    ) external virtual onlyRole(YIELD_RECIPIENT_MANAGER_ROLE) {
